...
void main()
{
	...
	gl_PointSize = mix(1.0/*min*/, 10.0/*max*/, 0.5 * sin(uTime) + 0.5);
	...
	gl_Position = uProjectionMatrix * uViewMatrix * uModelMatrix * vec4(position_in, 0.0, 1.0);
}
`;
...
function init_wgl()
{
	...
	UserInterface.begin(); // name of html id
		...
		// - sliders (name, min, max, default value, callback called when value is modified)
		// - update_wgl() is called to refresh screen
		slider_cameraZoom  = UserInterface.add_slider('Zoom', 1.5, 10, 3, update_wgl);
	UserInterface.end();
...
function draw_wgl()
{
	...
	// ---- "projection matrix" could be orthogonal (ex: architecture), perspective (ex: films, video games), or custom (ex: spherical)
	Uniforms.uProjectionMatrix = Matrix.perspective(3.1416/4/*fov: field of view*/, 16/9/*aspect ratio*/, 1/*near*/, 10/*far*/);
	// ---- "view matrix" is the view from the camera space (or eye space)
    //Uniforms.uViewMatrix = Matrix.look_at(Vec3(0,0,2)/*eye*/, Vec3(0,0,-1)/*dir*/, Vec3(0,1,0)/*up*/);
	Uniforms.uViewMatrix = Matrix.look_at(Vec3(0, 0, slider_cameraZoom.value)/*eye*/, Vec3(0, 0, -1)/*dir*/, Vec3(0, 1, 0)/*up*/);
	// - set model matrix
	// ---- animate rotation along z axis
