...
Afin d'éviter d'avoir à écrire des fichiers html/css pour chaque tp, on pourra utiliser le fichier __tp.html__ (attention à modifier le chemin d'accès au fichier _easy_wgl.js_ ligne 22). 
...
On peut la faire apparaitre/disparaitre en cliquant dans le coin bas-gauche de la fenêtre de rendu avec le bouton gauche (ou appel à `ewgl.console.toggle()` depuis la console de debuggage du browser. Avec le bouton droit au même endroit ça nettoie la console (appel de `ewgl.console.clear()`)
...
### Sous-échantillonnage et image color processing options

L'appel de ewgl.enable_sub_sampling(N) divise la définition de l'image par N
...
* `Matrix.mult(...)` : renvoit la multiplication de plusieurs Matrices. On peut même finir par un vecteur.
* `Matrix.translate(tx,ty,tz)` génère la matrice de translation 
* `Matrix.scale(sx,sy,sz)` génère la matrice d'homotéhtie
* `Matrix.rotateX(b)` génère la matrice de rotation autour de l'axe X d'angle b en degre
* `Matrix.rotateY(b)` génère la matrice de rotation autour de l'axe Y d'angle b en degre
* `Matrix.rotateZ(b)` génère la matrice de rotation autour de l'axe Z d'angle b en degre
* `Matrix.rotate(b,a)` génère la matrice de rotation autour de l'axe a d'angle b en degre
* `Matrix.ortho2D()` génère la matrice orthogonale,  pour resoect duratio hauteur/largeur
* `Matrix.ortho2D_2()` génère la matrice orthogonale (2x2)
* `Matrix.ortho(aspect, near, far)` génère la matrice de projection orthogonale
  * aspect ratio x/y
...
Pour utiliser un program-shader, utiliser la méthode `.bind()`.

Pour arrêter l'utilisation de tout shader, appeler la fonction `unbind_shader();`
...
Si l'uniform _name_ n'existe pas le message d'erreur est _... objet non extensible ..._
...
Pour utiliser une texture dans un shader, il faut lier la texture, activer un moteur de texture et passer le numéro du moteur à l'uniform de type _sampler2D_.
