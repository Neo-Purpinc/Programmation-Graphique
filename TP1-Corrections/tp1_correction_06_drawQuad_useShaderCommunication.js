...
// OUPUT
// - one can output data per vertex and communicate between shaders during traversal of the graphics pipeline
// - you can choose whatever name you want BUT name MUST be the same in all shaders !
// - variable is prefixed by "out" as it it "send through" the graphics pipeline
out vec2 vertexData;
...
{
		// OUTPUT DATA
	// - one can output data per vertex
	// - here, we send input positions through the pipeline
	vertexData = position_in;
...
// INPUT
// - one can retrieve data send between shaders during traversal of the graphics pipeline
// - you can choose whatever name you want BUT name MUST be the same in all shaders !
// - variable is prefixed by "in" as it it "received from" the graphics pipeline
in vec2 vertexData;
...
void main()
{
	...
	// - display color with vertexData received from vertex shader thruogh the graphics pipeline
	// - this value has been interpolated on primitives between vertices (visible on lines, not on points) with the help of GPU hardware "rasterizer"
	oFragmentColor = vec4(vertexData, 0.25, 1); // [values are between 0.0 and 1.0]
}
`;
